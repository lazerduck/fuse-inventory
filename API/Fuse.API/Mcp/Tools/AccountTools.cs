...
public sealed class AccountTools(IAccountService accounts, IIdentityService identities, McpToolAuthorization authorization)
{
    ...
    public async Task<object> CreateAccount(CreateAccount command, CancellationToken ct = default)
    { await Require(AccountPermissions.CreateKey, ct); return McpResult.Value(await accounts.CreateAccountAsync(command)); }
    [McpServerTool(Name = "inventory_replace_account", Destructive = true)]
    public async Task<object> ReplaceAccount(UpdateAccount command, CancellationToken ct = default)
    { await Require(AccountPermissions.UpdateKey, ct); return McpResult.Value(await accounts.UpdateAccountAsync(command)); }
    ...
    public async Task<object> CreateIdentity(CreateIdentity command, CancellationToken ct = default)
    { await Require(IdentityPermissions.CreateKey, ct); return McpResult.Value(await identities.CreateIdentityAsync(command)); }
    [McpServerTool(Name = "inventory_replace_identity", Destructive = true)]
    public async Task<object> ReplaceIdentity(UpdateIdentity command, CancellationToken ct = default)
    { await Require(IdentityPermissions.UpdateKey, ct); return McpResult.Value(await identities.UpdateIdentityAsync(command)); }
