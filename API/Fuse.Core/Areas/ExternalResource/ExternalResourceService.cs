...
public class ExternalResourceService : IExternalResourceService
{
    ...
    public async Task<IReadOnlyList<Models.ExternalResource>> GetExternalResourcesAsync()
        => (await _fuseStore.GetAsync()).ExternalResources;

    public async Task<Models.ExternalResource?> GetExternalResourceByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).ExternalResources.FirstOrDefault(r => r.Id == id);

    public async Task<Result<Models.ExternalResource>> CreateExternalResourceAsync(CreateExternalResource command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.ExternalResource>.Failure("Resource name cannot be empty.", ErrorType.Validation);
        ...
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<Models.ExternalResource>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
        ...
        var res = new Models.ExternalResource(
            Id: Guid.NewGuid(),
            Name: command.Name,
        ...
        return Result<Models.ExternalResource>.Success(res);
    }

    public async Task<Result<Models.ExternalResource>> UpdateExternalResourceAsync(UpdateExternalResource command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.ExternalResource>.Failure("Resource name cannot be empty.", ErrorType.Validation);
        ...
        if (existing is null)
            return Result<Models.ExternalResource>.Failure($"External resource with ID '{command.Id}' not found.", ErrorType.NotFound);
