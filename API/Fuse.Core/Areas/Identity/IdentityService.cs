...
public class IdentityService : IIdentityService
{
    ...
    public async Task<IReadOnlyList<Models.Identity>> GetIdentitiesAsync()
        => (await _fuseStore.GetAsync()).Identities;

    public async Task<Models.Identity?> GetIdentityByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).Identities.FirstOrDefault(i => i.Id == id);

    public async Task<Result<Models.Identity>> CreateIdentityAsync(CreateIdentity command)
    {
        ...
        if (!assignmentValidation.IsSuccess)
            return Result<Models.Identity>.Failure(assignmentValidation.Error!, assignmentValidation.ErrorType ?? ErrorType.Validation);
        ...
        return Result<Models.Identity>.Success(identity);
    }

    public async Task<Result<Models.Identity>> UpdateIdentityAsync(UpdateIdentity command)
    {
        var store = await _fuseStore.GetAsync();
        var tagIds = command.TagIds ?? new HashSet<Guid>();
        var existing = store.Identities.FirstOrDefault(i => i.Id == command.Id);
        if (existing is null)
            return Result<Models.Identity>.Failure($"Identity with ID '{command.Id}' not found.", ErrorType.NotFound);
    ...
    private async Task<Result<Models.Identity>?> ValidateIdentityCommand(string name, Guid? ownerInstanceId, HashSet<Guid> tagIds)
    {
        if (string.IsNullOrWhiteSpace(name))
            return Result<Models.Identity>.Failure("Identity name cannot be empty.", ErrorType.Validation);
        ...
        if (ownerInstanceId is Guid oid)
        {
            ...
            if (!allInstances.Any(i => i.Id == oid))
                return Result<Models.Identity>.Failure($"Owner instance with ID '{oid}' not found.", ErrorType.Validation);
        }
        ...
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<Models.Identity>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
    ...
    private static bool TargetExists(Snapshot s, TargetKind kind, Guid id) => kind switch
    {
        ...
        TargetKind.MessageBroker => s.MessageBrokers.Any(m => m.Id == id),
        _ => false
    };
    ...
    public async Task<Result<IReadOnlyList<Models.Identity>>> CloneIdentityAsync(CloneIdentity command)
    {
        var store = await _fuseStore.GetAsync();
        var source = store.Identities.FirstOrDefault(i => i.Id == command.SourceId);
        if (source is null)
            return Result<IReadOnlyList<Models.Identity>>.Failure($"Identity with ID '{command.SourceId}' not found.", ErrorType.NotFound);

        if (source.OwnerInstanceId is null)
            return Result<IReadOnlyList<Models.Identity>>.Failure("Cannot clone an identity without an owner instance.", ErrorType.Validation);

        if (command.TargetOwnerInstanceIds is null || command.TargetOwnerInstanceIds.Count == 0)
            return Result<IReadOnlyList<Models.Identity>>.Failure("At least one target instance must be specified.", ErrorType.Validation);

        var allInstances = store.Applications.SelectMany(a => a.Instances).ToList();
        var created = new List<Models.Identity>();
        var now = DateTime.UtcNow;

        foreach (var targetInstanceId in command.TargetOwnerInstanceIds)
        {
            if (!allInstances.Any(i => i.Id == targetInstanceId))
                return Result<IReadOnlyList<Models.Identity>>.Failure($"Target instance with ID '{targetInstanceId}' not found.", ErrorType.Validation);
        ...
        return Result<IReadOnlyList<Models.Identity>>.Success(created);
    }
}
