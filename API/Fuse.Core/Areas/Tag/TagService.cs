...
public class TagService : ITagService
{
    ...
    public async Task<Result<Models.Tag>> CreateTagAsync(CreateTag command)
    {
        if (command.Name == string.Empty)
        {
            return Result<Models.Tag>.Failure("Tag name cannot be empty.", ErrorType.Validation);
        }
        ...
        return Result<Models.Tag>.Success(newTag);
    }
    ...
    public async Task<Models.Tag?> GetTagByIdAsync(Guid id)
    {
        return (await _fuseStore.GetAsync()).Tags
            .FirstOrDefault(t => t.Id == id);
    }

    public async Task<IReadOnlyList<Models.Tag>> GetTagsAsync()
    {
        return (await _fuseStore.GetAsync()).Tags;
    }

    public async Task<Result<Models.Tag>> UpdateTagAsync(UpdateTag command)
    {
        if (command.Name == string.Empty)
        {
            return Result<Models.Tag>.Failure("Tag name cannot be empty.", ErrorType.Validation);
        ...
        {
            return Result<Models.Tag>.Failure($"Tag with ID '{command.Id}' not found.", ErrorType.NotFound);
        }
        ...
        return Result<Models.Tag>.Success(updatedTag);
    }
}
