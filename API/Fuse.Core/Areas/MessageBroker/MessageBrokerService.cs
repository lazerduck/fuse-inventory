...
public class MessageBrokerService : IMessageBrokerService
{
    ...
    public async Task<IReadOnlyList<Models.MessageBroker>> GetMessageBrokersAsync()
        => (await _fuseStore.GetAsync()).MessageBrokers;

    public async Task<Models.MessageBroker?> GetMessageBrokerByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).MessageBrokers.FirstOrDefault(m => m.Id == id);

    public async Task<Result<Models.MessageBroker>> CreateMessageBrokerAsync(CreateMessageBroker command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.MessageBroker>.Failure("Message broker name cannot be empty.", ErrorType.Validation);

        if (string.IsNullOrWhiteSpace(command.Kind))
            return Result<Models.MessageBroker>.Failure("Message broker kind cannot be empty.", ErrorType.Validation);

        var store = await _fuseStore.GetAsync();

        if (!store.Environments.Any(e => e.Id == command.EnvironmentId))
            return Result<Models.MessageBroker>.Failure($"Environment with ID '{command.EnvironmentId}' not found.", ErrorType.Validation);

        var tagIds = command.TagIds ?? new HashSet<Guid>();
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<Models.MessageBroker>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
        ...
        var broker = new Models.MessageBroker(
            Id: Guid.NewGuid(),
            Name: command.Name,
        ...
        return Result<Models.MessageBroker>.Success(broker);
    }

    public async Task<Result<Models.MessageBroker>> UpdateMessageBrokerAsync(UpdateMessageBroker command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.MessageBroker>.Failure("Message broker name cannot be empty.", ErrorType.Validation);
        ...
        if (existing is null)
            return Result<Models.MessageBroker>.Failure($"Message broker with ID '{command.Id}' not found.", ErrorType.NotFound);
