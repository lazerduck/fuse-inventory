...
public class SecretProviderService : ISecretProviderService
{
    ...
    public async Task<IReadOnlyList<Models.SecretProvider>> GetSecretProvidersAsync()
        => (await _fuseStore.GetAsync()).SecretProviders;

    public async Task<Models.SecretProvider?> GetSecretProviderByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).SecretProviders.FirstOrDefault(p => p.Id == id);

    public async Task<Result<Models.SecretProvider>> CreateSecretProviderAsync(CreateSecretProvider command)
    {
        ...
        if (!resolvedCredentialsResult.IsSuccess)
            return Result<Models.SecretProvider>.Failure(resolvedCredentialsResult.Error!, resolvedCredentialsResult.ErrorType ?? ErrorType.Validation);

        var validation = ValidateSecretProviderCommand(command.Name, command.VaultUri, command.Capabilities);
        if (!validation.IsSuccess)
            return Result<Models.SecretProvider>.Failure(validation.Error!, validation.ErrorType ?? ErrorType.Validation);
        ...
        if (!testResult.IsSuccess)
            return Result<Models.SecretProvider>.Failure($"Connection test failed: {testResult.Error}", ErrorType.Validation);
        ...
        var provider = new Models.SecretProvider(
            Id: Guid.NewGuid(),
            Name: command.Name,
        ...
        return Result<Models.SecretProvider>.Success(provider);
    }

    public async Task<Result<Models.SecretProvider>> UpdateSecretProviderAsync(UpdateSecretProvider command)
    {
        var store = await _fuseStore.GetAsync();
        var existing = store.SecretProviders.FirstOrDefault(p => p.Id == command.Id);
        if (existing is null)
            return Result<Models.SecretProvider>.Failure($"Secret provider with ID '{command.Id}' not found.", ErrorType.NotFound);
