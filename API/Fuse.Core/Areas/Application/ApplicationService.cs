...
public class ApplicationService : IApplicationService
{
    ...
    public async Task<IReadOnlyList<ApplicationModel>> GetApplicationsAsync() => (await _fuseStore.GetAsync()).Applications;

    public async Task<ApplicationModel?> GetApplicationByIdAsync(Guid id) => (await _fuseStore.GetAsync()).Applications.FirstOrDefault(a => a.Id == id);

    public async Task<Result<ApplicationModel>> CreateApplicationAsync(CreateApplication command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<ApplicationModel>.Failure("Application name cannot be empty.", ErrorType.Validation);

        var tagIds = command.TagIds ?? new HashSet<Guid>();
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<ApplicationModel>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
        ...
        return Result<ApplicationModel>.Success(updatedApp!);
    }

    public async Task<Result<ApplicationModel>> UpdateApplicationAsync(UpdateApplication command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<ApplicationModel>.Failure("Application name cannot be empty.", ErrorType.Validation);
        ...
        if (existing is null)
            return Result<ApplicationModel>.Failure($"Application with ID '{command.Id}' not found.", ErrorType.NotFound);
        ...
        return Result<ApplicationModel>.Success(updated);
    }
