...
public class DataStoreService : IDataStoreService
{
    ...
    public async Task<IReadOnlyList<Models.DataStore>> GetDataStoresAsync()
        => (await _fuseStore.GetAsync()).DataStores;

    public async Task<Models.DataStore?> GetDataStoreByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).DataStores.FirstOrDefault(d => d.Id == id);

    public async Task<Result<Models.DataStore>> CreateDataStoreAsync(CreateDataStore command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.DataStore>.Failure("Data store name cannot be empty.", ErrorType.Validation);
        if (string.IsNullOrWhiteSpace(command.Kind))
            return Result<Models.DataStore>.Failure("Data store kind cannot be empty.", ErrorType.Validation);

        var store = await _fuseStore.GetAsync();
        if (!store.Environments.Any(e => e.Id == command.EnvironmentId))
            return Result<Models.DataStore>.Failure($"Environment with ID '{command.EnvironmentId}' not found.", ErrorType.Validation);

        if (command.PlatformId is Guid pid)
        {
            var platform = store.Platforms.FirstOrDefault(s => s.Id == pid);
            if (platform is null)
                return Result<Models.DataStore>.Failure($"Platform with ID '{pid}' not found.", ErrorType.Validation);
        }
        ...
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<Models.DataStore>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
        ...
        return Result<Models.DataStore>.Success(ds);
    }

    public async Task<Result<Models.DataStore>> UpdateDataStoreAsync(UpdateDataStore command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.DataStore>.Failure("Data store name cannot be empty.", ErrorType.Validation);
        if (string.IsNullOrWhiteSpace(command.Kind))
        ...
        if (existing is null)
            return Result<Models.DataStore>.Failure($"Data store with ID '{command.Id}' not found.", ErrorType.NotFound);
