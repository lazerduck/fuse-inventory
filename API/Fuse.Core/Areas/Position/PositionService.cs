...
public class PositionService : IPositionService
{
    ...
    public async Task<IReadOnlyList<Models.Position>> GetPositionsAsync()
        => (await _fuseStore.GetAsync()).Positions;

    public async Task<Models.Position?> GetPositionByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).Positions.FirstOrDefault(p => p.Id == id);

    public async Task<Result<Models.Position>> CreatePositionAsync(CreatePosition command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.Position>.Failure("Position name cannot be empty.", ErrorType.Validation);

        var tagIds = command.TagIds ?? new HashSet<Guid>();
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<Models.Position>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
        ...
        return Result<Models.Position>.Success(position);
    }

    public async Task<Result<Models.Position>> UpdatePositionAsync(UpdatePosition command)
    {
        if (string.IsNullOrWhiteSpace(command.Name))
            return Result<Models.Position>.Failure("Position name cannot be empty.", ErrorType.Validation);
        ...
        if (existing is null)
            return Result<Models.Position>.Failure($"Position with ID '{command.Id}' not found.", ErrorType.NotFound);
        ...
        return Result<Models.Position>.Success(updated);
    }
