...
public class RiskService : IRiskService
{
    ...
    public async Task<IReadOnlyList<Models.Risk>> GetRisksAsync()
        => (await _fuseStore.GetAsync()).Risks;

    public async Task<Models.Risk?> GetRiskByIdAsync(Guid id)
        => (await _fuseStore.GetAsync()).Risks.FirstOrDefault(r => r.Id == id);

    public async Task<IReadOnlyList<Models.Risk>> GetRisksByTargetAsync(string targetType, Guid targetId)
        => (await _fuseStore.GetAsync()).Risks
            .Where(r => r.TargetType == targetType && r.TargetId == targetId)
            .ToList();

    public async Task<Result<Models.Risk>> CreateRiskAsync(CreateRisk command)
    {
        ...
        return Result<Models.Risk>.Success(risk);
    }

    public async Task<Result<Models.Risk>> UpdateRiskAsync(UpdateRisk command)
    {
        var store = await _fuseStore.GetAsync();
        var tagIds = command.TagIds ?? new HashSet<Guid>();
        var existing = store.Risks.FirstOrDefault(r => r.Id == command.Id);
        if (existing is null)
            return Result<Models.Risk>.Failure($"Risk with ID '{command.Id}' not found.", ErrorType.NotFound);
        ...
        return Result<Models.Risk>.Success(updated);
    }
    ...
    private async Task<Result<Models.Risk>?> ValidateRiskCommand(Guid ownerPositionId, Guid? approverPositionId, string targetType, Guid targetId, HashSet<Guid> tagIds)
    {
        var store = await _fuseStore.GetAsync();
        ...
        if (!store.Positions.Any(p => p.Id == ownerPositionId))
            return Result<Models.Risk>.Failure($"Owner position with ID '{ownerPositionId}' not found.", ErrorType.Validation);
        ...
        if (approverPositionId.HasValue && !store.Positions.Any(p => p.Id == approverPositionId.Value))
            return Result<Models.Risk>.Failure($"Approver position with ID '{approverPositionId}' not found.", ErrorType.Validation);
        ...
        if (!targetExists)
            return Result<Models.Risk>.Failure($"Target {targetType} with ID '{targetId}' not found.", ErrorType.Validation);

        // Validate tags exist
        foreach (var tagId in tagIds)
        {
            if (await _tagService.GetTagByIdAsync(tagId) is null)
                return Result<Models.Risk>.Failure($"Tag with ID '{tagId}' not found.", ErrorType.Validation);
        }
