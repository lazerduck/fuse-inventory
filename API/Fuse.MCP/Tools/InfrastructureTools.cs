    ...
    IPlatformService platforms, ITagService tags, McpToolAuthorization authorization)
{
    ...
    public async Task<object> CreatePlatform(string displayName, string? dnsName = null, string? os = null, PlatformKind? kind = null, string? ipAddress = null, string? notes = null, IReadOnlyList<Guid>? tagIds = null, CancellationToken ct = default) { await Require(PlatformPermissions.CreateKey, ct); return McpResult.Value(await platforms.CreatePlatformAsync(new(displayName, dnsName, os, kind, ipAddress, notes, tagIds?.ToHashSet()))); }
    [McpServerTool(Name = "inventory_replace_platform", Destructive = true)]
    public async Task<object> ReplacePlatform(Guid platformId, string displayName, string? dnsName = null, string? os = null, PlatformKind? kind = null, string? ipAddress = null, string? notes = null, IReadOnlyList<Guid>? tagIds = null, CancellationToken ct = default) { await Require(PlatformPermissions.UpdateKey, ct); return McpResult.Value(await platforms.UpdatePlatformAsync(new(platformId, displayName, dnsName, os, kind, ipAddress, notes, tagIds?.ToHashSet()))); }
